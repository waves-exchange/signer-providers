"use strict";
var __defProp = Object.defineProperty;
var __defProps = Object.defineProperties;
var __getOwnPropDescs = Object.getOwnPropertyDescriptors;
var __getOwnPropSymbols = Object.getOwnPropertySymbols;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __propIsEnum = Object.prototype.propertyIsEnumerable;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    if (__hasOwnProp.call(b, prop))
      __defNormalProp(a, prop, b[prop]);
  if (__getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(b)) {
      if (__propIsEnum.call(b, prop))
        __defNormalProp(a, prop, b[prop]);
    }
  return a;
};
var __spreadProps = (a, b) => __defProps(a, __getOwnPropDescs(b));
var __objRest = (source, exclude) => {
  var target = {};
  for (var prop in source)
    if (__hasOwnProp.call(source, prop) && exclude.indexOf(prop) < 0)
      target[prop] = source[prop];
  if (source != null && __getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(source)) {
      if (exclude.indexOf(prop) < 0 && __propIsEnum.call(source, prop))
        target[prop] = source[prop];
    }
  return target;
};
var __publicField = (obj, key, value) => {
  __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  return value;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports[Symbol.toStringTag] = "Module";
var BigNumber = require("@waves/bignumber");
var React = require("react");
var ramda = require("ramda");
require("@waves/ts-types");
var reactUikit = require("@waves.exchange/react-uikit");
var core = require("@emotion/core");
var wavesTransactions = require("@waves/waves-transactions");
function _interopDefaultLegacy(e) {
  return e && typeof e === "object" && "default" in e ? e : { "default": e };
}
var BigNumber__default = /* @__PURE__ */ _interopDefaultLegacy(BigNumber);
var React__default = /* @__PURE__ */ _interopDefaultLegacy(React);
const MAX_ALIAS_LENGTH = 30;
const WAVES = {
  ticker: "WAVES",
  assetId: "WAVES",
  name: "Waves",
  decimals: 8,
  description: "",
  issueHeight: 0,
  issueTimestamp: new Date("2016-04-11T21:00:00.000Z").getTime(),
  reissuable: false,
  scripted: false,
  minSponsoredFee: null,
  quantity: BigNumber.BigNumber.toBigNumber(1e8).mul(Math.pow(10, 8)).toFixed(),
  issuer: "WAVES",
  minSponsoredAssetFee: null,
  logo: ""
};
const NAME_MAP = {
  issue: 3,
  transfer: 4,
  reissue: 5,
  burn: 6,
  exchange: 7,
  lease: 8,
  cancelLease: 9,
  alias: 10,
  massTransfer: 11,
  data: 12,
  setScript: 13,
  sponsorship: 14,
  setAssetScript: 15,
  invoke: 16,
  updateAssetInfo: 17
};
const SPONSORED_TYPES = [
  NAME_MAP.transfer,
  NAME_MAP.invoke
];
var constantsModule = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  [Symbol.toStringTag]: "Module",
  MAX_ALIAS_LENGTH,
  WAVES,
  NAME_MAP,
  SPONSORED_TYPES
});
const useHandleFeeSelect = (tx) => {
  const [txJSON, setTxJSON] = React.useState(JSON.stringify(tx, null, 2));
  React.useEffect(() => setTxJSON(JSON.stringify(tx, null, 2)), [tx, tx.id]);
  return [
    React.useCallback((fee, feeAssetId) => {
      tx.fee = fee;
      tx["feeAssetId"] = feeAssetId;
      setTxJSON(JSON.stringify(tx, null, 2));
    }, [tx]),
    txJSON
  ];
};
var hooksModule = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  [Symbol.toStringTag]: "Module",
  useHandleFeeSelect
});
const onContentLoad = new Promise((resolve2) => {
  document.addEventListener("DOMContentLoaded", resolve2);
});
function loadScript(url) {
  return onContentLoad.then(() => new Promise((resolve2, reject) => {
    const script = document.createElement("script");
    script.addEventListener("load", resolve2);
    script.addEventListener("error", reject);
    script.src = url;
    document.head.appendChild(script);
  }));
}
function hasScript(url) {
  return !!document.querySelector('script[src="' + url + '"]');
}
function runByPath(path, args) {
  let obj = window;
  const parts = path.split(".");
  const isFunc = obj[path] && typeof obj[path] === "function";
  if (parts.length === 1 && isFunc) {
    return obj[path].apply(obj, args);
  }
  parts.slice(-1).forEach(function(part) {
    obj = obj[part];
  });
  if (obj[parts[parts.length - 1]] && typeof obj[parts[parts.length - 1]] === "function") {
    obj[parts[parts.length - 1]].apply(obj, args);
  }
}
const _Analytics = class {
  constructor() {
    __publicField(this, "loaded");
    __publicField(this, "events", []);
    __publicField(this, "apiList", []);
    __publicField(this, "defaultParams", Object.create(null));
    __publicField(this, "isActive", false);
    __publicField(this, "isActivated", false);
    __publicField(this, "apiReadyList", []);
    if (_Analytics.instance) {
      return _Analytics.instance;
    }
    _Analytics.instance = this;
  }
  init(defaultParams) {
    Object.assign(this.defaultParams, defaultParams);
  }
  addApi(data) {
    this.apiList.push(data);
  }
  addDefaultParams(params) {
    Object.assign(this.defaultParams, params);
  }
  deactivate() {
    this.isActive = false;
  }
  activate() {
    if (!this.isActivated) {
      const apiLoadList = this.apiList.map((item) => {
        return loadScript(item.libraryUrl).then(() => {
          runByPath(item.initializeMethod, [
            item.apiToken,
            item.initializeOptions
          ]);
        }).then(() => {
          this.apiReadyList.push({
            type: item.type,
            send: function(name, params) {
              runByPath(item.sendMethod, [name, params]);
            }
          });
          return true;
        }).catch(() => {
          console.error("Invalid analytics config", item);
          return false;
        });
      });
      this.loaded = Promise.all(apiLoadList);
    }
    this.loaded.then(() => {
      const events = [...this.events];
      this.events = [];
      events.forEach((event) => this._sendEvent(event));
      this.isActive = true;
    });
    this.isActivated = true;
  }
  send(data) {
    const event = __spreadProps(__spreadValues({}, data), {
      params: __spreadValues(__spreadValues({}, this.defaultParams), data.params)
    });
    if (this.isActive && this.isActivated) {
      this._sendEvent(event);
    } else {
      this.events.push(event);
    }
  }
  _sendEvent(data) {
    if (!data.name) {
      throw new Error("Wrong format, has no event name!");
    }
    const type = data.target || "all";
    this.apiReadyList.filter(function(item) {
      return type === "all" ? true : item.type === type;
    }).forEach(function(item) {
      return item.send(data.name, __spreadValues({}, data.params));
    });
  }
};
let Analytics = _Analytics;
__publicField(Analytics, "instance");
const analytics = new Analytics();
const assetPropFactory = (assets) => (assetId, property) => ramda.isNil(assetId) ? ramda.prop(property, WAVES) : ramda.prop(property, assets[assetId]);
const cleanAddress = (address) => {
  return address.replace(/alias:.:/, "");
};
function isAddress(address) {
  return cleanAddress(address).length > MAX_ALIAS_LENGTH;
}
const fixRecipient = ramda.curry((networkByte, data) => {
  const address = cleanAddress(data.recipient);
  if (isAddress(address)) {
    return __spreadProps(__spreadValues({}, data), { recipient: address });
  } else {
    return __spreadProps(__spreadValues({}, data), {
      recipient: `alias:${String.fromCharCode(networkByte)}:${address}`
    });
  }
});
const getEnvAwareUrl = (pathname) => {
  const origin = window.location.origin.includes("localhost") ? "https://waves.exchange" : window.location.origin;
  if (!pathname)
    return origin;
  if (!pathname.startsWith("/"))
    throw new Error("Pathname should start with /");
  return `${origin}${pathname}`;
};
const getTxAliases = (tx) => {
  switch (tx.type) {
    case NAME_MAP.invoke:
      return isAddress(tx.dApp) ? [] : [tx.dApp];
    case NAME_MAP.transfer:
    case NAME_MAP.lease:
      return isAddress(tx.recipient) ? [] : [tx.recipient];
    case NAME_MAP.massTransfer:
      return tx.transfers.reduce((acc, transfer) => {
        if (!isAddress(transfer.recipient)) {
          acc.push(transfer.recipient);
        }
        return acc;
      }, []);
    default:
      return [];
  }
};
const isAlias = (address) => {
  return cleanAddress(address).length <= MAX_ALIAS_LENGTH;
};
function isSafari() {
  const userAgent = navigator.userAgent.toLowerCase();
  const isSafariUA = userAgent.includes("safari") && !userAgent.includes("chrome");
  const iOS = navigator.platform != null && /iPad|iPhone|iPod/.test(navigator.platform);
  return iOS || isSafariUA;
}
function isBrave() {
  var _a;
  return !!((_a = navigator.brave) == null ? void 0 : _a.isBrave);
}
function resolve(path, base) {
  return new URL(path, base).toString();
}
const reg = new RegExp('((?!\\\\)"\\w+"):\\s*(-?[\\d|\\.]{14,})', "g");
function parse(json) {
  return JSON.parse(json.replace(reg, `$1:"$2"`));
}
const request = typeof fetch === "function" ? fetch : require("node-fetch");
function request$1(params) {
  return request(resolve(params.url, params.base), updateHeaders(params.options)).then(parseResponse);
}
function parseResponse(r) {
  return r.text().then((message) => r.ok ? parse(message) : Promise.reject(tryParse(message)));
}
function tryParse(message) {
  try {
    return JSON.parse(message);
  } catch (e) {
    return message;
  }
}
function updateHeaders(options = Object.create(null)) {
  return Object.assign({ credentials: "include" }, options);
}
function isObject(obj) {
  if (typeof obj === "object" && obj !== null) {
    if (typeof Object.getPrototypeOf === "function") {
      const prototype = Object.getPrototypeOf(obj);
      return prototype === Object.prototype || prototype === null;
    }
    return Object.prototype.toString.call(obj) === "[object Object]";
  }
  return false;
}
const keys = (obj) => Object.keys(obj);
const deepAssign = (...objects) => objects.reduce((target, merge) => {
  keys(merge).forEach((key) => {
    if (Array.isArray(target[key]) && Array.isArray(merge[key])) {
      target[key] = Array.from(new Set(target[key].concat(merge[key])));
    } else if (isObject(target[key]) && isObject(merge[key])) {
      target[key] = deepAssign(target[key], merge[key]);
    } else {
      target[key] = merge[key];
    }
  });
  return target;
}, objects[0] || {});
const FIELDS = ["amount", "matcherFee", "price", "fee", "minSponsoredAssetFee", "quantity", "sellMatcherFee", "buyMatcherFee"];
function stringify(data) {
  return JSON.stringify(data, function(key, value) {
    if (FIELDS.includes(key)) {
      return `!${value}!`;
    } else if (key === "value" && this["type"] === "integer") {
      return `!${value}!`;
    } else {
      return value;
    }
  }, 0).replace(/"\!(-?\d+)\!"/g, "$1");
}
function fetchCalculateFee(base, tx, options = Object.create(null)) {
  return request$1({
    base,
    url: "/transactions/calculateFee",
    options: deepAssign(Object.assign({}, options), {
      method: "POST",
      body: stringify(tx),
      headers: {
        "Content-Type": "application/json"
      }
    })
  });
}
const loadFeeByTransaction = ramda.curry((base, tx) => fetchCalculateFee(base, tx).then((info) => __spreadProps(__spreadValues({}, tx), { fee: info.feeAmount })).catch(() => __spreadValues({}, tx)));
const loadLogoInfo = ramda.curry((networkByte, data) => Promise.all(data.map((asset) => {
  const network = String.fromCharCode(networkByte) === "W" ? "" : "testnet.";
  const fetchLogoUrl = `https://${network}waves.exchange/static/icons/assets/${asset.assetId}.svg`;
  return fetch(fetchLogoUrl).then((response) => {
    if (response.ok) {
      return __spreadProps(__spreadValues({}, asset), { logo: fetchLogoUrl });
    }
    return asset;
  });
})));
const getPrintableNumber = (number, decimals) => {
  return BigNumber__default["default"].toBigNumber(number).div(Math.pow(10, decimals)).roundTo(decimals).toFixed();
};
const getCoins = (n, decimals) => {
  return BigNumber__default["default"].toBigNumber(n).mul(Math.pow(10, decimals)).toFixed();
};
class Queue {
  constructor(maxLength) {
    __publicField(this, "_actions", []);
    __publicField(this, "_maxLength");
    __publicField(this, "_active");
    this._maxLength = maxLength;
  }
  get length() {
    return this._actions.length + (this._active == null ? 0 : 1);
  }
  push(action) {
    if (this._actions.length >= this._maxLength) {
      throw new Error("Cant't push action! Queue is full!");
    }
    return new Promise((resolve2, reject) => {
      const onEnd = () => {
        this._active = void 0;
        const index = this._actions.map(ramda.prop("action")).indexOf(actionCallback);
        if (index !== -1) {
          this._actions.splice(index, 1);
        }
        this.run();
      };
      const actionCallback = () => action().then(ramda.pipe(ramda.tap(onEnd), resolve2), ramda.pipe(ramda.tap(onEnd), reject));
      this._actions.push({ action: actionCallback, reject });
      if (this.length === 1) {
        this.run();
      }
    });
  }
  clear(error) {
    error = error || new Error("Rejection with clear queue!");
    const e = typeof error === "string" ? new Error(error) : error;
    this._actions.splice(0, this._actions.length).forEach((item) => item.reject(e));
    this._active = void 0;
  }
  canPush() {
    return this._actions.length < this._maxLength;
  }
  run() {
    const item = this._actions.shift();
    if (item == null) {
      return void 0;
    }
    this._active = item.action();
  }
}
const toArray = (data) => Array.isArray(data) ? data : [data];
function toFormat(num, id, hash) {
  const asset = id != null ? hash[id] : WAVES;
  if (asset == null) {
    throw new Error("Asset not found!");
  }
  return `${getPrintableNumber(num, asset.decimals)} ${asset.name}`;
}
var utilsModule = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  [Symbol.toStringTag]: "Module",
  onContentLoad,
  loadScript,
  hasScript,
  runByPath,
  analytics,
  assetPropFactory,
  cleanAddress,
  fixRecipient,
  getEnvAwareUrl,
  getTxAliases,
  isAddress,
  isAlias,
  isSafari,
  isBrave,
  loadFeeByTransaction,
  loadLogoInfo,
  getPrintableNumber,
  getCoins,
  Queue,
  toArray,
  toFormat
});
const getAlias = (address) => {
  return isAlias(address) ? cleanAddress(address) : "";
};
const getIconType = (tx, user, userAliases) => {
  if (tx.type === 11)
    return "mass";
  const isSenderMe = tx.senderPublicKey == null || tx.senderPublicKey === user.publicKey;
  if (isAlias(tx.recipient)) {
    return isSenderMe ? userAliases.includes(getAlias(tx.recipient)) ? "circular" : "send" : "receive";
  } else {
    return isSenderMe ? tx.recipient === user.address ? "circular" : "send" : "receive";
  }
};
var iconTransferHelpers = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  [Symbol.toStringTag]: "Module",
  getIconType
});
const Confirmation = (_a) => {
  var _b = _a, {
    address,
    canConfirm = true,
    name,
    balance,
    children,
    onReject,
    onConfirm
  } = _b, rest = __objRest(_b, [
    "address",
    "canConfirm",
    "name",
    "balance",
    "children",
    "onReject",
    "onConfirm"
  ]);
  return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, __spreadValues({
    backgroundColor: "main.$800",
    width: "520px",
    flexDirection: "column",
    justifyContent: "space-between",
    borderRadius: "$6",
    boxShadow: "0 0 30px rgba(0, 0, 0, 0.15)"
  }, rest), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    justifyContent: "space-between",
    alignItems: "center",
    height: "65px",
    borderBottom: "1px solid",
    borderBottomColor: "basic.$1000"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    mr: "$10"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
    isShort: true,
    address,
    name,
    withCopy: true
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address,
    variantSize: "large"
  }))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column",
    alignItems: "flex-end"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "footnote1",
    color: "basic.$500"
  }, "Balance"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0"
  }, balance))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    maxHeight: "630px",
    overflowY: "auto"
  }, children), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    borderTop: "1px solid",
    borderTopColor: "main.$700",
    px: "$40",
    py: "$20"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Button, {
    flexGrow: 1,
    variant: "danger",
    variantSize: "medium",
    onClick: onReject
  }, "Reject"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Button, {
    ml: "$20",
    flexGrow: 1,
    variant: "primary",
    variantSize: "medium",
    disabled: !canConfirm,
    onClick: onConfirm
  }, "Confirm")));
};
const DataEntry = ({ data }) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  mb: "$20",
  px: "15px",
  bg: "basic.$900",
  borderRadius: "$4"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  py: "13px",
  borderBottom: "1px solid",
  borderColor: "main.$500"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "35%"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Key test")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "50%"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Value")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "15%"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Type"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  maxHeight: "165px",
  overflow: "hidden",
  overflowY: "auto",
  mr: "5px"
}, data.map((item, i) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  key: item.key,
  borderTop: i === 0 ? 0 : "1px dashed",
  borderColor: "main.$500",
  py: "13px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "35%"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0",
  isTruncated: true,
  display: "inline-block",
  maxWidth: "100%",
  pr: "$20",
  verticalAlign: "middle"
}, item.key)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "50%"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0",
  isTruncated: true,
  display: "inline-block",
  maxWidth: "100%",
  pr: "$20",
  verticalAlign: "middle"
}, String(item.value || ""))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "15%"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, item["type"] || ""))))));
const DataJson = ({ data }) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  borderRadius: "$4",
  bg: "basic.$900",
  p: "15px",
  maxHeight: "165px",
  overflowY: "auto",
  color: "standard.$0"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  as: "pre",
  m: "0",
  variant: "body2",
  fontFamily: "Menlo, Monaco, Consolas, Courier New, monospace"
}, typeof data === "string" ? data : JSON.stringify(data, null, 2)));
const checkIsEnoughBalance = (balance, fee) => {
  return BigNumber.BigNumber.toBigNumber(balance).gte(BigNumber.BigNumber.toBigNumber(fee).div(Math.pow(10, WAVES.decimals)));
};
const hasParamsFee = (fee) => typeof fee === "string" || typeof fee === "number";
const isWaves = (feeId) => feeId === "WAVES";
const isFeeAssetId = (feeAssetId) => typeof feeAssetId !== "undefined";
const isNonDefaultFeeAssetId = (feeAssetId) => typeof feeAssetId === "string";
const formatFee = (fee, decimals) => getPrintableNumber(fee, decimals);
const getFeeOptions = ({
  txFee,
  txMeta,
  paramsFeeAssetId,
  availableWavesBalance,
  paramsFee: txParamsFee
}) => {
  const getAssetProp = assetPropFactory(txMeta.assets);
  const wavesFeeOption = {
    id: null,
    name: WAVES.name,
    ticker: WAVES.ticker,
    value: formatFee(txFee, WAVES.decimals)
  };
  let defaultFeeOption;
  let feeAsset;
  if (!isWaves(paramsFeeAssetId) && (isFeeAssetId(paramsFeeAssetId) || hasParamsFee(txParamsFee))) {
    if (isNonDefaultFeeAssetId(paramsFeeAssetId)) {
      feeAsset = txMeta.assets[paramsFeeAssetId];
      defaultFeeOption = {
        name: getAssetProp(paramsFeeAssetId, "name"),
        id: paramsFeeAssetId,
        ticker: "",
        value: formatFee(txFee, feeAsset.decimals)
      };
    } else {
      defaultFeeOption = wavesFeeOption;
    }
    return [defaultFeeOption];
  }
  defaultFeeOption = wavesFeeOption;
  const metaFeeOptions = txMeta.feeList.map((f) => ({
    name: getAssetProp(f.feeAssetId, "name"),
    id: String(f.feeAssetId),
    ticker: "",
    value: formatFee(f.feeAmount, getAssetProp(f.feeAssetId, "decimals"))
  }));
  const isEnoughBalance = checkIsEnoughBalance(availableWavesBalance, txFee);
  const hasNonDefaultFees = metaFeeOptions.length > 0;
  let feeOptions;
  if (hasNonDefaultFees) {
    feeOptions = isEnoughBalance ? [defaultFeeOption].concat(metaFeeOptions) : metaFeeOptions.filter(({ id }) => id !== WAVES.assetId);
  } else {
    feeOptions = [defaultFeeOption];
  }
  return feeOptions;
};
const FeeSelect = (_c) => {
  var _d = _c, {
    txMeta,
    fee: txFee,
    onFeeSelect,
    availableWavesBalance,
    as: _as
  } = _d, rest = __objRest(_d, [
    "txMeta",
    "fee",
    "onFeeSelect",
    "availableWavesBalance",
    "as"
  ]);
  const txFeeAssetId = txMeta.params.feeAssetId;
  const getAssetProp = assetPropFactory(txMeta.assets);
  const [feeOptions, setFeeOptions] = React.useState(getFeeOptions({
    txFee,
    paramsFee: txMeta.params.fee,
    txMeta,
    paramsFeeAssetId: txFeeAssetId,
    availableWavesBalance
  }));
  React.useCallback(() => {
    setFeeOptions(getFeeOptions({
      txFee,
      paramsFee: txMeta.params.fee,
      txMeta,
      paramsFeeAssetId: txFeeAssetId,
      availableWavesBalance
    }));
  }, [availableWavesBalance, txFee, txFeeAssetId, txMeta]);
  const [selectedFeeOption, setSelectedFeeOption] = React.useState(feeOptions[0]);
  React.useEffect(() => {
    setSelectedFeeOption(feeOptions[0]);
  }, [feeOptions]);
  React.useEffect(() => {
    if (selectedFeeOption) {
      const feeAssetId = selectedFeeOption.id;
      const fee = getCoins(selectedFeeOption.value, getAssetProp(selectedFeeOption.id, "decimals"));
      onFeeSelect(fee, feeAssetId);
    }
  }, [selectedFeeOption]);
  const handleFeeSelect = React.useCallback((feeOption) => {
    setSelectedFeeOption(feeOption);
  }, [getAssetProp, onFeeSelect]);
  return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, __spreadValues({}, rest), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    display: "block",
    mb: "$5"
  }, "Fee"), feeOptions.length > 1 ? /* @__PURE__ */ React__default["default"].createElement(reactUikit.Select, {
    placement: "top",
    renderSelected: (open) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Selected, {
      selected: selectedFeeOption,
      opened: open
    })
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.List, {
    onSelect: handleFeeSelect,
    options: feeOptions
  })) : /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0"
  }, selectedFeeOption.value, " ", selectedFeeOption.name));
};
const Help = (_e) => {
  var _f = _e, {
    children,
    popperOptions
  } = _f, rest = __objRest(_f, [
    "children",
    "popperOptions"
  ]);
  return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tooltip, __spreadValues({
    arrowSize: "4px",
    hasArrow: true,
    arrowColor: "#5A81EA",
    offset: 4,
    maxWidth: "360px",
    label: () => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
      borderTop: "4px solid",
      borderTopColor: "primary.$300",
      backgroundColor: "#4a5060",
      borderRadius: "$4",
      p: 15,
      color: "standard.$0",
      width: "max-content",
      maxWidth: "400px"
    }, children),
    placement: "bottom",
    popperOptions: __spreadProps(__spreadValues({}, popperOptions), {
      modifiers: [
        {
          name: "flip",
          enabled: false
        },
        ...popperOptions && popperOptions["modifiers"] || []
      ]
    })
  }, rest), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    size: "14px",
    backgroundColor: "basic.$500",
    borderRadius: "circle",
    justifyContent: "center",
    alignItems: "center",
    cursor: "pointer"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    iconSize: "potty",
    icon: reactUikit.iconQuestion,
    color: "standard.$1000"
  })));
};
const IconTransfer = (_g) => {
  var _h = _g, { type } = _h, rest = __objRest(_h, ["type"]);
  switch (type) {
    case "mass":
      return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, __spreadValues({
        icon: reactUikit.iconMassTransfer,
        color: "#FFAF00"
      }, rest));
    case "send":
      return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, __spreadValues({
        icon: reactUikit.iconTransferArrow,
        color: "#FFAF00"
      }, rest));
    case "receive":
      return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, __spreadValues({
        sx: { transform: "rotate(180deg)" },
        icon: reactUikit.iconTransferArrow,
        color: "#81C926"
      }, rest));
    case "circular":
      return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, __spreadValues({
        icon: reactUikit.iconTransferCircle,
        color: "#EF4829"
      }, rest));
    default:
      return null;
  }
};
const COLOR_MAP = {
  integer: "#b5cea9",
  string: "#cf9178",
  binary: "#cf9178",
  boolean: "#579cd6"
};
const pseudoElemStyles = {
  color: "#d4d4d4",
  fontSize: "$13",
  lineHeight: "$18",
  fontFamily: "Menlo, Monaco, Consolas, Courier New, monospace",
  display: "inline"
};
const wrapperStylesStart = {
  ":before": __spreadProps(__spreadValues({}, pseudoElemStyles), {
    content: '"("'
  })
};
const wrapperStylesEnd = {
  ":after": __spreadProps(__spreadValues({}, pseudoElemStyles), {
    content: '")"'
  })
};
const getAttrStyles = (attrType, isLast) => ({
  variant: "body2",
  fontFamily: "Menlo, Monaco, Consolas, Courier New, monospace",
  color: COLOR_MAP[attrType],
  ":after": !isLast ? __spreadProps(__spreadValues({}, pseudoElemStyles), { content: '",\xA0"' }) : {}
});
const formatText = (text) => String(text).length >= 5 ? `${String(text).slice(0, 4)}...` : String(text);
const getAttrContent = (type, value) => {
  switch (type) {
    case "integer":
      return formatText(value);
    case "boolean":
      return String(value);
    case "string":
      return `'${formatText(value)}'`;
    case "binary":
      return "'base64:...'";
    default:
      return null;
  }
};
const InvokeFunction = (_i) => {
  var _j = _i, {
    args,
    name
  } = _j, rest = __objRest(_j, [
    "args",
    "name"
  ]);
  return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, __spreadValues({
    sx: wrapperStylesEnd,
    fontSize: "$13",
    lineHeight: "$18"
  }, rest), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    isTruncated: true
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    sx: pseudoElemStyles
  }, name), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    sx: wrapperStylesStart
  }, args.map(({ type, value }, index) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    key: index,
    sx: getAttrStyles(type, index === args.length - 1)
  }, getAttrContent(type, value))))));
};
const InvokePayment = ({
  assetId,
  amount,
  name,
  logo,
  isLast
}) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  color: "standard.$0",
  p: "$10",
  borderBottomWidth: isLast ? "0" : "1px",
  borderBottomColor: "main.$500",
  borderBottomStyle: "dashed"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AssetLogo, {
  mr: "$10",
  logo,
  assetId,
  name,
  size: 26
}), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2"
}, name), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  flex: "2",
  textAlign: "right"
}, amount));
const TransactionDetails = ({ tx }) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  justifyContent: "space-between",
  py: "14px",
  borderBottom: "1px solid",
  borderTop: "1px solid",
  borderColor: "main.$700"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Type"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, tx.type)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  justifyContent: "space-between",
  py: "14px",
  borderBottom: "1px solid",
  borderColor: "main.$700"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Version"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, tx.version)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  justifyContent: "space-between",
  py: "14px",
  borderBottom: "1px solid",
  borderColor: "main.$700"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "TX Id"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, tx.id)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  justifyContent: "space-between",
  py: "14px",
  borderBottom: "1px solid",
  borderColor: "main.$700"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Timestamp"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, tx.timestamp)));
var Roboto = "data:font/woff2;base64,";
var RobotoLight = "data:font/woff2;base64,";
var RobotoMedium = "data:font/woff2;base64,";
const GlobalCSS = () => {
  return /* @__PURE__ */ React__default["default"].createElement(core.Global, {
    styles: [
      {
        "@font-face": {
          fontFamily: "Roboto",
          fontStyle: "normal",
          fontWeight: 300,
          fontDisplay: "swap",
          src: `url(${RobotoLight}) format("woff2")`
        }
      },
      {
        "@font-face": {
          fontFamily: "Roboto",
          fontStyle: "normal",
          fontWeight: 400,
          fontDisplay: "swap",
          src: `url(${Roboto}) format("woff2")`
        }
      },
      {
        "@font-face": {
          fontFamily: "Roboto",
          fontStyle: "normal",
          fontWeight: 700,
          fontDisplay: "swap",
          src: `url(${RobotoMedium}) format("woff2")`
        }
      },
      {
        "*": {
          boxSizing: "border-box"
        },
        html: {
          height: "100%"
        },
        body: {
          fontFamily: "Roboto, sans-serif",
          margin: 0,
          padding: 0,
          minHeight: "100vh",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          WebkitOverflowScrolling: "touch"
        },
        "@supports (-webkit-touch-callout: none)": {
          body: {
            minHeight: "-webkit-fill-available"
          }
        },
        button: {
          fontFamily: "Roboto, sans-serif"
        },
        "#root": {
          position: "relative",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          width: "520px",
          minHeight: "100%",
          maxHeight: ["100vh", "initial"]
        },
        "#overlay": {
          position: "fixed",
          zIndex: -1,
          top: 0,
          left: 0,
          height: "100%",
          width: "100%",
          overflow: "hidden",
          backgroundColor: "#000",
          opacity: 0.6
        },
        "::-webkit-scrollbar-thumb": {
          backgroundColor: "#495060",
          borderRadius: "4px"
        },
        "::-webkit-scrollbar": {
          width: "3px",
          height: "3px"
        },
        "::-webkit-scrollbar-corner": {
          opacity: 0
        },
        "::-webkit-scrollbar-track-piece": {
          marginBottom: "5px",
          marginTop: "5px"
        },
        "::placeholder": {
          color: "#959dae"
        }
      }
    ]
  });
};
const iconTransferUtils = __spreadValues({}, iconTransferHelpers);
const getAmountAsset = (assetId, assets) => assetId === null || assetId === void 0 ? WAVES : assets[assetId];
const getAssetName = (assetId, assets) => {
  return assetId === null || assetId === "WAVES" || assetId === void 0 ? WAVES.name : assets[assetId].name;
};
const getFeeAsset = (txType, assets, txFeeAssetId) => {
  if (txFeeAssetId === "WAVES" || txFeeAssetId === null || typeof txFeeAssetId === "undefined") {
    return WAVES;
  }
  return assets[txFeeAssetId];
};
const getRecipientAddress = (recipient, aliases) => isAlias(recipient) ? aliases[recipient] : recipient;
const getRawTransfersList = (aliases, massTransfers) => {
  return massTransfers.map(({ amount, recipient }) => ({
    name: recipient,
    address: getRecipientAddress(recipient, aliases),
    amount
  }));
};
const getTotalTransferAmount = (rawTransferList, decimals) => {
  const sum = rawTransferList.reduce((sum2, { amount }) => {
    return BigNumber__default["default"].toBigNumber(amount).add(sum2);
  }, BigNumber__default["default"].toBigNumber(0));
  return getPrintableNumber(sum.toString(), decimals);
};
const getTransferList = (rawTransferList, decimals) => rawTransferList.map((item) => __spreadProps(__spreadValues({}, item), {
  amount: getPrintableNumber(item.amount, decimals)
}));
const getFeeAssetName = (txType, assets, feeAssetId) => {
  return txType === 11 || feeAssetId === "undefined" || feeAssetId === null ? WAVES.name : getAssetName(getFeeAsset(txType, assets, feeAssetId).assetId, assets);
};
const getPrintableTxFee = ({
  txType,
  txFee,
  assets,
  txFeeAssetId
}) => `${getPrintableNumber(txFee, getFeeAsset(txType, assets, txFeeAssetId).decimals)} ${getFeeAssetName(txType, assets, txFeeAssetId)}`;
const getTransferViewData = ({
  txRecipient,
  txAssetId,
  txFee,
  txFeeAssetId,
  txAmount,
  assets,
  aliases
}) => {
  const { name, decimals } = getAmountAsset(txAssetId, assets);
  return {
    totalTransferAmount: `${getPrintableNumber(txAmount, decimals)} ${name}`,
    transferList: [
      {
        name: txRecipient,
        address: getRecipientAddress(txRecipient, aliases),
        amount: getPrintableNumber(txAmount, decimals)
      }
    ],
    fee: getPrintableTxFee({
      txType: NAME_MAP.transfer,
      txFee,
      assets,
      txFeeAssetId
    })
  };
};
const getMassTransferViewData = ({
  txTransfers,
  txAssetId,
  txFee,
  assets,
  aliases
}) => {
  const { name, decimals } = getAmountAsset(txAssetId, assets);
  const rawTransferList = getRawTransfersList(aliases, txTransfers);
  return {
    totalTransferAmount: `${getTotalTransferAmount(rawTransferList, decimals)} ${name}`,
    transferList: getTransferList(rawTransferList, decimals),
    fee: getPrintableTxFee({
      txType: NAME_MAP.transfer,
      txFee,
      assets
    })
  };
};
const getViewData = (tx, { aliases, assets }) => {
  const transferViewData = tx.type === 4 ? getTransferViewData({
    aliases,
    assets,
    txAssetId: tx.assetId,
    txRecipient: tx.recipient,
    txAmount: tx.amount,
    txFee: tx.fee,
    txFeeAssetId: tx.feeAssetId
  }) : getMassTransferViewData({
    aliases,
    assets,
    txAssetId: tx.assetId,
    txTransfers: tx.transfers,
    txFee: tx.fee
  });
  let attachment = "";
  try {
    attachment = wavesTransactions.libs.crypto.bytesToString(wavesTransactions.libs.crypto.base58Decode((tx == null ? void 0 : tx.attachment) || ""));
  } catch (e) {
  }
  return __spreadProps(__spreadValues({}, transferViewData), {
    attachment
  });
};
const isTransferMeta = (meta) => meta.params.type === NAME_MAP.transfer;
var transferHelpers = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  [Symbol.toStringTag]: "Module",
  getAmountAsset,
  getAssetName,
  getFeeAsset,
  getRecipientAddress,
  getRawTransfersList,
  getTotalTransferAmount,
  getTransferList,
  getFeeAssetName,
  getPrintableTxFee,
  getTransferViewData,
  getMassTransferViewData,
  getViewData,
  isTransferMeta
});
function batchPage(props) {
  return /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("div", {
    onClick: props.onCancel
  }), /* @__PURE__ */ React__default["default"].createElement("div", {
    className: "logo"
  }), /* @__PURE__ */ React__default["default"].createElement("div", null, "Confirm TX"), /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("span", null, "Sign from"), /* @__PURE__ */ React__default["default"].createElement("span", null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: props.user.address
  }))), /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("span", null, "Type"), /* @__PURE__ */ React__default["default"].createElement("span", null, "Batch")), /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("span", null, "Transaction Count"), /* @__PURE__ */ React__default["default"].createElement("span", null, props.list.length)), /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("button", {
    onClick: props.onCancel
  }, "Cancel"), /* @__PURE__ */ React__default["default"].createElement("button", {
    onClick: props.onConfirm
  }, "Ok")));
}
const SignAliasComponent = ({
  userAddress,
  userName,
  userBalance,
  userHasScript,
  tx,
  fee,
  onConfirm,
  onReject
}) => {
  const {
    boundaryRef: helpTooltipBoundaryRef,
    popperOptions: helpTooltipPopperOptions
  } = reactUikit.useBoundedTooltip({
    left: 60,
    right: 60
  });
  const { boundaryRef, popperOptions } = reactUikit.useBoundedTooltip({});
  return /* @__PURE__ */ React__default["default"].createElement("div", {
    ref: helpTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: userBalance,
    onReject,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(90, 174, 249, 0.1)",
    height: 60,
    width: 60
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconAliasTransaction,
    size: 40,
    color: "#00aef9"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    flexDirection: "column",
    justifyContent: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: 26,
    lineHeight: "32px",
    color: "standard.$0"
  }, "Sign Alias TX"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, {
    ref: boundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    mb: "$5"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mr: "$5"
  }, "Alias"), /* @__PURE__ */ React__default["default"].createElement(Help, {
    showDelay: 1500,
    popperOptions: __spreadProps(__spreadValues({
      strategy: "fixed"
    }, helpTooltipPopperOptions), {
      modifiers: [
        {
          name: "flip",
          enabled: false
        },
        ...helpTooltipPopperOptions && helpTooltipPopperOptions["modifiers"] || []
      ]
    })
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    maxWidth: "400px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    color: "standard.$0"
  }, "About Alias"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    as: "p"
  }, "An Alias is a nickname for your address. You can use an Alias instead of an address to make transactions."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    as: "p"
  }, "Your Alias must be between 4 and 30 characters long and must contain only lowercase Latin letters, digits and symbols (@, -, _) and dot.")))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
    address: userAddress,
    alias: tx.alias,
    withCopy: true,
    mb: "$20"
  }, userHasScript ? /* @__PURE__ */ React__default["default"].createElement(reactUikit.BoxWithIcon, {
    icon: reactUikit.iconSmartMini,
    iconLabel: "Smart Account",
    popperOptions
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: userAddress,
    variantSize: "large"
  })) : /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: userAddress,
    variantSize: "large"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$5",
    display: "block"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    display: "block"
  }, fee)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  }))))));
};
const SignBurn = ({
  userAddress,
  userName,
  userBalance,
  tx,
  fee,
  onCancel,
  onConfirm,
  burnAmount,
  assetId,
  assetName,
  isSmartAsset
}) => {
  const { boundaryRef, popperOptions } = reactUikit.useBoundedTooltip({ left: 40 });
  return /* @__PURE__ */ React__default["default"].createElement("div", {
    ref: boundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: userBalance,
    onReject: onCancel,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(255, 175, 0, 0.1)",
    height: 60,
    width: 60,
    flexShrink: 0
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconBurnTransaction,
    size: 40,
    color: "#FFAF00"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    flexDirection: "column",
    overflowX: "hidden"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    color: "basic.$500"
  }, "Sign Burn TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: 26,
    lineHeight: "32px",
    color: "standard.$0",
    isTruncated: true
  }, burnAmount))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column",
    bg: "main.$800"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500"
  }, "Asset ID"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    mt: "$5"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AssetLogoWithIcon, {
    icon: reactUikit.iconSmartMini,
    iconLabel: "Smart asset",
    iconVisible: isSmartAsset,
    assetId,
    name: assetName,
    size: 30,
    flexShrink: 0,
    popperOptions
  }), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Copy, {
    ml: "$10",
    inititialTooltipLabel: "Copy Asset ID",
    copiedTooltipLabel: "Copied!",
    text: assetId
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    isTruncated: true
  }, assetId))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mt: "$20",
    display: "block"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    display: "block",
    mt: "$5"
  }, fee))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  }))))));
};
const SignCancelLeaseComponent = ({
  userAddress,
  userBalance,
  userName,
  amount,
  tx,
  fee,
  onReject,
  onConfirm
}) => /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
  address: userAddress,
  name: userName,
  balance: userBalance,
  onReject,
  onConfirm
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$20",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  justifyContent: "center",
  borderRadius: "circle",
  bg: "rgba(239, 72, 41, 0.1)",
  height: 60,
  width: 60
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
  icon: reactUikit.iconCancelLeaseTransaction,
  size: 40,
  color: "#ef4829"
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  ml: "$20",
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1",
  color: "basic.$500"
}, "Sign Lease Cancel TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  fontSize: 26,
  lineHeight: "32px",
  color: "standard.$0"
}, amount))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
  borderBottom: "1px solid",
  borderColor: "main.$700",
  bg: "main.$900",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
  bg: "main.$800",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  mt: "$20",
  variant: "body2",
  color: "basic.$500"
}, "Lease ID"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Copy, {
  inititialTooltipLabel: "Copy Lease ID",
  copiedTooltipLabel: "Copied!",
  text: tx.leaseId,
  mb: "$20"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0",
  display: "block"
}, tx.leaseId)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  mt: "$20",
  variant: "body2",
  color: "basic.$500"
}, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0",
  display: "block"
}, fee)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
  tx
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
  data: tx
})))));
const SignDataComponent = ({
  userAddress,
  userName,
  userBalance,
  tx,
  fee,
  onConfirm,
  onReject
}) => /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
  address: userAddress,
  name: userName,
  balance: userBalance,
  onReject,
  onConfirm
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$20",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  justifyContent: "center",
  borderRadius: "circle",
  bg: "rgba(90, 129, 234, 0.1)",
  height: 60,
  width: 60
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
  icon: reactUikit.iconDataTransaction,
  size: 40,
  color: "primary.$300"
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  ml: "$20",
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1",
  color: "basic.$500"
}, "On-chain"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  fontSize: 26,
  lineHeight: "32px",
  color: "standard.$0"
}, "Sign Data TX"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
  borderBottom: "1px solid",
  borderColor: "main.$700",
  bg: "main.$900",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
  bg: "main.$800",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500",
  display: "block",
  mb: "$5"
}, "Data"), /* @__PURE__ */ React__default["default"].createElement(DataEntry, {
  data: tx.data
}), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500",
  mb: "$5",
  display: "block"
}, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0",
  display: "block"
}, fee)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
  tx
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
  data: tx
})))));
const SignInvoke = ({
  userAddress,
  userName,
  userBalance,
  dAppAddress,
  dAppName,
  meta,
  call,
  payment,
  tx,
  onCancel,
  onConfirm,
  handleFeeSelect,
  txJSON
}) => {
  var _a, _b;
  const { boundaryRef, popperOptions } = reactUikit.useBoundedTooltip({});
  return /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: `${getPrintableNumber(userBalance, WAVES.decimals)} Waves`,
    onReject: onCancel,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    py: "$20",
    px: "$40",
    bg: "main.$900"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    borderRadius: "circle",
    width: "60px",
    height: "60px",
    bg: "rgba(255, 175, 0, 0.1)",
    justifyContent: "center",
    alignItems: "center",
    mr: "$20"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    size: "40px",
    icon: reactUikit.iconInvoke,
    color: "#FFAF00"
  })), payment && /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    justifyContent: "center",
    flexDirection: "column"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    color: "basic.$500",
    mb: "$3"
  }, "Sign Invoke Script TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Heading, {
    variant: "heading2",
    color: "standard.$0"
  }, payment.length > 0 ? payment.length : "No", " ", "Payments"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    mb: "$20",
    ref: boundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$3",
    as: "div"
  }, "dApp"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
    address: dAppAddress,
    alias: dAppName,
    withCopy: true,
    mt: "$5"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.BoxWithIcon, {
    icon: reactUikit.iconSmartMini,
    iconLabel: "Script account",
    popperOptions
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: dAppAddress
  })))), payment && payment.length > 0 && /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    mb: "$20"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    as: "div",
    mb: "$5"
  }, "Payments"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    p: "$5",
    bg: "basic.$900",
    borderRadius: "$4"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column",
    borderRadius: "$4",
    bg: "basic.$900",
    px: "$5",
    maxHeight: "165px",
    overflowY: "auto"
  }, payment.map((pay, i) => /* @__PURE__ */ React__default["default"].createElement(InvokePayment, __spreadProps(__spreadValues({
    key: pay.assetId || "WAVES"
  }, pay), {
    isLast: i === payment.length - 1
  })))))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    mb: "$20"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    as: "div",
    mb: "$5"
  }, "Call function"), /* @__PURE__ */ React__default["default"].createElement(InvokeFunction, {
    borderRadius: "$4",
    bg: "basic.$900",
    p: "15px",
    color: "basic.$500",
    as: "div",
    overflowX: "auto",
    args: (_a = call == null ? void 0 : call.args) != null ? _a : [],
    name: (_b = call == null ? void 0 : call.function) != null ? _b : "default"
  })), /* @__PURE__ */ React__default["default"].createElement(FeeSelect, {
    mt: "$20",
    txMeta: meta,
    fee: tx.fee,
    onFeeSelect: handleFeeSelect,
    availableWavesBalance: userBalance
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: txJSON
  })))));
};
const SignIssueComponent = ({
  fee,
  assetId,
  assetName,
  assetDescription,
  assetType,
  assetScript,
  decimals,
  userAddress,
  userName,
  userBalance,
  issueAmount,
  onReject,
  onConfirm,
  onTermsCheck,
  canConfirm,
  tx
}) => {
  const {
    boundaryRef: helpTooltipBoundaryRef,
    popperOptions: helpTooltipPopperOptions
  } = reactUikit.useBoundedTooltip({
    left: 60,
    right: 60
  });
  const {
    boundaryRef: assetLogoWithIconTooltipBoundaryRef,
    popperOptions: assetLogoWithIconPopperOptions
  } = reactUikit.useBoundedTooltip({ left: 0 });
  return /* @__PURE__ */ React__default["default"].createElement("div", {
    ref: helpTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: userBalance,
    onReject,
    onConfirm,
    canConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(38, 193, 201, 0.1)",
    height: 60,
    width: 60,
    flexShrink: 0
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconIssueTransaction,
    size: 40,
    color: "#26c1c9"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    flexDirection: "column",
    overflowX: "hidden"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    color: "basic.$500"
  }, "Sign Issue TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: 26,
    lineHeight: "32px",
    color: "standard.$0",
    isTruncated: true
  }, issueAmount))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column",
    bg: "main.$800",
    ref: assetLogoWithIconTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500"
  }, "Asset ID"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    mt: "$5"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AssetLogoWithIcon, {
    assetId,
    name: assetName,
    size: "30px",
    flexShrink: 0,
    icon: reactUikit.iconSmartMini,
    iconVisible: Boolean(assetScript),
    iconLabel: "Smart asset",
    popperOptions: assetLogoWithIconPopperOptions
  }), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Copy, {
    ml: "$10",
    inititialTooltipLabel: "Copy Asset ID",
    copiedTooltipLabel: "Copied!",
    text: assetId
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    isTruncated: true
  }, assetId))), assetDescription ? /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    mt: "$20",
    variant: "body2",
    color: "basic.$500"
  }, "Asset Description"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    mt: "$5",
    p: "15px",
    variant: "body2",
    color: "standard.$0",
    bg: "basic.$900",
    borderRadius: "$4",
    maxHeight: "85px",
    overflowY: "auto"
  }, assetDescription)) : null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    mt: 25
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    mt: "$25",
    mr: "3px",
    variant: "body2",
    color: "basic.$500"
  }, "Asset Type"), /* @__PURE__ */ React__default["default"].createElement(Help, {
    popperOptions: helpTooltipPopperOptions
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    fontWeight: 700,
    display: "block"
  }, "This field defines the total tokens supply that your asset will contain."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    display: "block",
    mt: "5px"
  }, "Reissuability allows for additional tokens creation that will be added to the total token supply of asset."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    display: "block",
    mt: "5px"
  }, "A non-reissuable asset will be permanently limited to the total token supply defined during this steps."))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    mt: "5px",
    variant: "body2",
    color: "standard.$0"
  }, assetType)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: 30,
    flexDirection: "column"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    mt: "$25",
    mr: "3px",
    variant: "body2",
    color: "basic.$500"
  }, "Decimals"), /* @__PURE__ */ React__default["default"].createElement(Help, {
    popperOptions: helpTooltipPopperOptions
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    fontWeight: 700
  }, "This field defines the number of decimals your asset token will be divided in."))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    mt: "5px",
    variant: "body2",
    color: "standard.$0"
  }, decimals))), assetScript ? /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    mr: "3px",
    mt: 25
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mr: "3px"
  }, "Smart Asset Script"), /* @__PURE__ */ React__default["default"].createElement(Help, {
    showDelay: 1500,
    popperOptions: helpTooltipPopperOptions
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body1",
    fontWeight: 700
  }, "A Smart Asset is an asset with an attached script that places conditions on every transaction made for the token in question."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    mt: "5px"
  }, "Each validation of a transaction by a Smart Asset's script increases the transaction fee by 0.004 WAVES. For example, if a regular tx is made for a Smart Asset, the cost is 0.001 + 0.004 = 0.005 WAVES. If an exchange transaction is made, the cost is 0.003 + 0.004 = 0.007 WAVES.", /* @__PURE__ */ React__default["default"].createElement(reactUikit.ExternalLink, {
    mt: "$5",
    variant: "body2",
    display: "block",
    href: "https://docs.wavesplatform.com/en/building-apps/smart-contracts/smart-assets",
    target: "_blank"
  }, "Learn more")))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    mt: "$5",
    p: "15px",
    variant: "body2",
    color: "standard.$0",
    bg: "basic.$900",
    borderRadius: "$4",
    maxHeight: "185px",
    overflowY: "auto"
  }, assetScript)) : null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    p: 15,
    border: "1px dashed",
    borderColor: "main.$500",
    backgroundColor: "main.$800",
    mt: 20
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body1",
    color: "standard.$0"
  }, "You agree that:"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    mt: "5px",
    variant: "body2",
    color: "basic.$300"
  }, "I) You will not use the token for fraudulent purposes;"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    color: "basic.$300"
  }, "II) You will not duplicate, fully or in part, the name of an existing cryptocurrency or a well-known company with the aim of misleading users;"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    color: "basic.$300"
  }, "III) You will not use names of states, other administrative units or municipal insitutions for the token's name with the aim of misleading users;"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    color: "basic.$300"
  }, "IV) You will not set a script on a smart asset that limits exchange transactions on Waves.Exchange by asset quantity;"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    color: "basic.$300"
  }, "V) You will not give false information in a smart asset's description concerning the rules governing the token's use, which do not correspond to those of the script installed on it;")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "flex-start",
    mt: 20
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Checkbox, {
    id: "terms",
    onChange: onTermsCheck
  }), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Label, {
    htmlFor: "terms",
    pl: "10px",
    color: "basic.$500",
    textAlign: "justify",
    fontSize: "13px",
    lineHeight: "18px"
  }, 'I understand that in the case of non-compliance with the rules, my token will be categorised as "Suspicious", and will be available for search only by ID')), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mt: "$20",
    display: "block"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    display: "block",
    mt: "$5"
  }, fee))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  }))))));
};
const SignLeaseComponent = ({
  recipientAddress,
  recipientName,
  userAddress,
  userBalance,
  userName,
  tx,
  amount,
  fee,
  onReject,
  onConfirm
}) => /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
  address: userAddress,
  name: userName,
  balance: userBalance,
  onReject,
  onConfirm
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$20",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  justifyContent: "center",
  borderRadius: "circle",
  bg: "rgba(129, 201, 38, 0.1)",
  height: 60,
  width: 60
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
  icon: reactUikit.iconLeaseTransaction,
  size: 40,
  color: "#81C926"
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  ml: "$20",
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1",
  color: "basic.$500"
}, "Sign Lease TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  fontSize: 26,
  lineHeight: "32px",
  color: "standard.$0"
}, amount))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
  borderBottom: "1px solid",
  borderColor: "main.$700",
  bg: "main.$900",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
  bg: "main.$800",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, "Recipient"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
  address: recipientAddress,
  alias: recipientName,
  withCopy: true,
  mt: "$5"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
  address: recipientAddress
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  mt: "$20",
  variant: "body2",
  color: "basic.$500"
}, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0",
  display: "block"
}, fee))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
  tx
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
  data: tx
})))));
const SignMessageComponent = ({
  userAddress,
  userName,
  userBalance,
  data,
  onReject,
  onConfirm
}) => /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
  address: userAddress,
  name: userName,
  balance: userBalance,
  onReject,
  onConfirm
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$20",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  justifyContent: "center",
  borderRadius: "circle",
  bg: "rgba(255, 175, 0, 0.1)",
  height: 60,
  width: 60
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
  size: "60px",
  icon: reactUikit.iconSignMessage,
  color: "primary.$300"
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  ml: "$20",
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1",
  color: "basic.$500"
}, "Off-chain"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  fontSize: 26,
  lineHeight: "32px",
  color: "standard.$0"
}, "Sign Message"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$30",
  flexDirection: "column",
  bg: "main.$800",
  borderTop: "1px solid",
  borderTopColor: "basic.$1000"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500",
  mb: "$5"
}, "Message"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  bg: "basic.$900",
  borderRadius: "$4",
  py: "15px",
  pl: "15px",
  pr: "5px",
  flexDirection: "column",
  alignItems: "flex-end"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  width: "100%",
  maxHeight: "147px",
  overflow: "hidden",
  overflowY: "auto",
  pr: "10px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, data)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.LightCopy, {
  text: data,
  mr: "15px",
  fontSize: 13,
  color: "primary.$300",
  lineHeight: "18px",
  cursor: "pointer"
}, "Copy"))));
const SignReissueComponent = ({
  userAddress,
  userName,
  userBalance,
  tx,
  reissueAmount,
  reissueAsset,
  fee,
  onConfirm,
  onReject
}) => {
  const {
    boundaryRef: assetLogoWithIconTooltipBoundaryRef,
    popperOptions: assetLogoWithIconPopperOptions
  } = reactUikit.useBoundedTooltip({ left: 0 });
  return /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: userBalance,
    onReject,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(129, 201, 38, 0.1)",
    height: 60,
    width: 60
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconReissueTransaction,
    size: 40,
    color: "#81c926"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    flexDirection: "column"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    color: "basic.$500"
  }, "Sign Reissue TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: 26,
    lineHeight: "32px",
    color: "standard.$0"
  }, reissueAmount))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    mb: "$20",
    ref: assetLogoWithIconTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$5",
    display: "block"
  }, "Asset ID"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AssetLogoWithIcon, {
    assetId: reissueAsset.assetId,
    name: reissueAsset.name,
    size: "30px",
    flexShrink: 0,
    icon: reactUikit.iconSmartMini,
    iconVisible: reissueAsset.scripted,
    iconLabel: "Smart asset",
    popperOptions: assetLogoWithIconPopperOptions
  }), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Copy, {
    text: reissueAsset.assetId,
    inititialTooltipLabel: "Copy",
    copiedTooltipLabel: "Copied!",
    ml: "$10"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    color: "standard.$0",
    variant: "body2"
  }, reissueAsset.assetId)))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    mb: "$5"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mr: "$5"
  }, "Asset Type"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Help, {
    align: "left",
    direction: "bottom"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    maxWidth: "400px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1",
    color: "standard.$0"
  }, "This field defines the total tokens supply that your asset will contain."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    as: "p"
  }, "Reissuability allows for additional tokens creation that will be added to the total token supply of asset."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    as: "p"
  }, "A non-reissuable asset will be permanently limited to the total token supply defined during these steps.")))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    mb: "$20",
    display: "block"
  }, tx.reissuable ? "Reissuable" : "Non-reissuable"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$5",
    display: "block"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    display: "block"
  }, fee)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  })))));
};
const SignSetAccountScriptComponent = ({
  userAddress,
  userName,
  userBalance,
  userHasScript,
  tx,
  fee,
  accountScript,
  onCancel,
  onConfirm
}) => {
  const {
    boundaryRef: helpTooltipBoundaryRef,
    popperOptions: helpTooltipPopperOptions
  } = reactUikit.useBoundedTooltip({
    left: 60,
    right: 60
  });
  const { boundaryRef, popperOptions } = reactUikit.useBoundedTooltip({});
  return /* @__PURE__ */ React__default["default"].createElement("div", {
    ref: helpTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: userBalance,
    onReject: onCancel,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900",
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(129, 201, 38, 0.1)",
    height: 60,
    width: 60,
    flexShrink: 0
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconSetAssetScript,
    size: 40,
    color: "#81C926"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    flexDirection: "column",
    overflowX: "hidden"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: 26,
    lineHeight: "32px",
    color: "standard.$0"
  }, "Sign Set Account Script TX"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column",
    bg: "main.$800"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500"
  }, "Account"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    mt: "$5",
    ref: boundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
    address: userAddress,
    withCopy: true
  }, userHasScript ? /* @__PURE__ */ React__default["default"].createElement(reactUikit.BoxWithIcon, {
    icon: reactUikit.iconSmartMini,
    iconLabel: "Smart Account",
    popperOptions
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: userAddress,
    variantSize: "large"
  })) : /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: userAddress,
    variantSize: "large"
  }))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    mt: "$20",
    mb: "$5",
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mr: "3px"
  }, "Smart Account Script"), /* @__PURE__ */ React__default["default"].createElement(Help, {
    showDelay: 1500,
    popperOptions: __spreadProps(__spreadValues({
      strategy: "fixed"
    }, helpTooltipPopperOptions), {
      modifiers: [
        {
          name: "flip",
          enabled: false
        },
        ...helpTooltipPopperOptions && helpTooltipPopperOptions["modifiers"] || []
      ]
    })
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body1",
    fontWeight: 700
  }, "A smart account is an account that has an account script attached to it."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    mt: "5px"
  }, "If the transaction is sent from a smart account, the transaction fee is increased by 0.004 WAVES. So if the transaction fee is 0.001 WAVES, the owner of the smart account will pay 0.001 + 0.004 = 0.005 WAVES", /* @__PURE__ */ React__default["default"].createElement(reactUikit.ExternalLink, {
    mt: "$5",
    variant: "body2",
    display: "block",
    href: "https://docs.wavesplatform.com/en/building-apps/smart-contracts/what-is-smart-account#expression"
  }, "Learn more")))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    bg: "basic.$900",
    borderRadius: "$4",
    py: "15px",
    pl: "15px",
    pr: "5px",
    flexDirection: "column",
    alignItems: "flex-end"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
    width: "100%",
    maxHeight: "165px",
    overflow: "auto",
    pr: "10px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    as: "pre",
    variant: "body2",
    color: "standard.$0",
    m: 0
  }, accountScript)), /* @__PURE__ */ React__default["default"].createElement(reactUikit.LightCopy, {
    text: accountScript || "",
    mr: "15px",
    fontSize: 13,
    color: "primary.$300",
    lineHeight: "18px",
    cursor: "pointer"
  }, "Copy")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mt: "$20",
    display: "block"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    display: "block",
    mt: "$5"
  }, fee))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  }))))));
};
const SignSetAssetScript = ({
  userAddress,
  userName,
  userBalance,
  tx,
  fee,
  onCancel,
  onConfirm,
  assetId,
  assetName,
  assetScript
}) => {
  const {
    boundaryRef: helpTooltipBoundaryRef,
    popperOptions: helpTooltipPopperOptions
  } = reactUikit.useBoundedTooltip({
    left: 60,
    right: 60
  });
  const {
    boundaryRef: smartAssetLogoTooltipBoundaryRef,
    popperOptions: smartAssetLogoPopperOptions
  } = reactUikit.useBoundedTooltip({ left: 0 });
  return /* @__PURE__ */ React__default["default"].createElement("div", {
    ref: helpTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: userBalance,
    onReject: onCancel,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900",
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(129, 201, 38, 0.1)",
    height: 60,
    width: 60,
    flexShrink: 0
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconSetAssetScript,
    size: 40,
    color: "#81C926"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    flexDirection: "column",
    overflowX: "hidden"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: 26,
    lineHeight: "32px",
    color: "standard.$0"
  }, "Sign Set Asset Script TX"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column",
    bg: "main.$800",
    ref: smartAssetLogoTooltipBoundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500"
  }, "Asset ID"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    mt: "$5"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AssetLogoWithIcon, {
    icon: reactUikit.iconSmartMini,
    iconLabel: "Smart asset",
    iconVisible: Boolean(assetScript),
    assetId,
    name: assetName,
    size: "30px",
    flexShrink: 0,
    popperOptions: __spreadValues({
      strategy: "fixed"
    }, smartAssetLogoPopperOptions)
  }), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Copy, {
    ml: "$10",
    inititialTooltipLabel: "Copy Asset ID",
    copiedTooltipLabel: "Copied!",
    text: assetId
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    isTruncated: true
  }, assetId))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    mt: "$20",
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mr: "3px"
  }, "Smart Asset Script"), /* @__PURE__ */ React__default["default"].createElement(Help, {
    showDelay: 1500,
    popperOptions: __spreadProps(__spreadValues({
      strategy: "fixed"
    }, helpTooltipPopperOptions), {
      modifiers: [
        {
          name: "flip",
          enabled: false
        },
        ...helpTooltipPopperOptions && helpTooltipPopperOptions["modifiers"] || []
      ]
    })
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body1",
    fontWeight: 700
  }, "A Smart Asset is an asset with an attached script that places conditions on every transaction made for the token in question."), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    variant: "body2",
    mt: "5px"
  }, "Each validation of a transaction by a Smart Asset's script increases the transaction fee by 0.004 WAVES. For example, if a regular tx is made for a Smart Asset, the cost is 0.001 + 0.004 = 0.005 WAVES. If an exchange transaction is made, the cost is 0.003 + 0.004 = 0.007 WAVES.", /* @__PURE__ */ React__default["default"].createElement(reactUikit.ExternalLink, {
    mt: "$5",
    variant: "body2",
    display: "block",
    href: "https://docs.wavesplatform.com/en/building-apps/smart-contracts/smart-assets"
  }, "Learn more")))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    display: "block",
    mt: "$5",
    p: "15px",
    variant: "body2",
    color: "standard.$0",
    bg: "basic.$900",
    borderRadius: "$4",
    maxHeight: "185px",
    overflowY: "auto"
  }, assetScript), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mt: "$20",
    display: "block"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0",
    display: "block",
    mt: "$5"
  }, fee))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  }))))));
};
const SignSponsorshipComponent = ({
  userAddress,
  userBalance,
  userName,
  tx,
  fee,
  sponsorAsset,
  sponsorCharge,
  isSponsorshipEnable,
  onReject,
  onConfirm
}) => {
  const { boundaryRef, popperOptions } = reactUikit.useBoundedTooltip({ left: 40 });
  return /* @__PURE__ */ React__default["default"].createElement("div", {
    ref: boundaryRef
  }, /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
    address: userAddress,
    name: userName,
    balance: `${getPrintableNumber(userBalance, WAVES.decimals)} Waves`,
    onReject,
    onConfirm
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    px: "$40",
    py: "$20",
    bg: "main.$900"
  }, isSponsorshipEnable ? /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(129, 201, 38, 0.1)",
    height: 60,
    width: 60
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconSponsorshipEnableTransaction,
    size: 40,
    color: "#81c926"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: "26px",
    lineHeight: "32px",
    color: "standard.$0"
  }, "Sign Enable Sponsorship TX"))) : /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: "circle",
    bg: "rgba(248, 183, 0, 0.1)",
    height: 60,
    width: 60
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
    icon: reactUikit.iconSponsorshipDisableTransaction,
    size: 40,
    color: "warning.$500"
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    ml: "$20",
    alignItems: "center"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    fontSize: "26px",
    lineHeight: "32px",
    color: "standard.$0"
  }, "Sign Disable Sponsorship TX")))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
    borderBottom: "1px solid",
    borderColor: "main.$700",
    bg: "main.$900",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
    mr: "32px",
    pb: "12px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body1"
  }, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
    bg: "main.$800",
    mb: "$30",
    px: "$40"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    flexDirection: "column"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$5"
  }, "Asset Id"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    alignItems: "center",
    mb: "$20"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AssetLogoWithIcon, {
    assetId: sponsorAsset.assetId,
    name: sponsorAsset.name,
    icon: reactUikit.iconSponsorshipMini,
    iconLabel: "Sponsorship",
    iconVisible: Number(sponsorAsset.minSponsoredAssetFee) > 0,
    logo: "logo" in sponsorAsset ? sponsorAsset.logo : void 0,
    size: "30px",
    flexShrink: 0,
    popperOptions
  }), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Copy, {
    text: sponsorAsset.assetId,
    inititialTooltipLabel: "Copy",
    copiedTooltipLabel: "Copied!",
    ml: "$10"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    color: "standard.$0",
    variant: "body2"
  }, sponsorAsset.assetId))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$5"
  }, "Sponsor Transaction Charge"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    color: "standard.$0",
    variant: "body2",
    mb: "$20"
  }, sponsorCharge), isSponsorshipEnable && /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    p: "15px",
    mb: "20px",
    flexDirection: "column",
    border: "1px dashed",
    borderColor: "#ffaf00",
    borderRadius: "$4"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    color: "#ffaf00",
    variant: "body1",
    mb: "$5"
  }, "Users will be able to pay transaction fees in", " ", sponsorAsset.name), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    color: "basic.$300",
    variant: "body2"
  }, "For each transaction, 0.001 WAVES will be deducted from your balance and ", sponsorAsset.name, " will be credited to your account. If you have the script account you will be cost 0.001 + 0.004 WAVES. The amount of ", sponsorAsset.name, " to be charged to users for transactions is set by you when you enable Sponsorship (enter the amount in the form above). Sponsorship will only work if you have a balance of at least 1.005 WAVES. If your balance falls below this amount, you will be unable to sponsor transactions.")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "basic.$500",
    mb: "$5"
  }, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
    variant: "body2",
    color: "standard.$0"
  }, fee))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
    tx
  })), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
    data: tx
  }))))));
};
const SignTransfer = ({
  userAddress,
  userBalance,
  userName,
  attachment,
  transferAmount,
  iconType,
  tx,
  meta,
  onReject,
  onConfirm,
  handleFeeSelect,
  txJSON,
  transferList,
  transferFee,
  isMassTransfer
}) => /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
  address: userAddress,
  name: userName,
  balance: `${getPrintableNumber(userBalance, WAVES.decimals)} Waves`,
  onReject,
  onConfirm
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$20",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  justifyContent: "center",
  borderRadius: "circle",
  bg: "rgba(255, 175, 0, 0.1)",
  height: 60,
  width: 60
}, /* @__PURE__ */ React__default["default"].createElement(IconTransfer, {
  type: iconType,
  size: 40
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  ml: "$20",
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1",
  color: "basic.$500"
}, !isMassTransfer ? "Sign Transfer TX" : "Sign MassTransfer TX"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  fontSize: 26,
  lineHeight: "32px",
  color: "standard.$0"
}, transferAmount))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
  borderBottom: "1px solid",
  borderColor: "main.$700",
  bg: "main.$900",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Main")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Details")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
  bg: "main.$800",
  mb: "$30",
  px: "$40"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500"
}, !isMassTransfer ? "Recipient" : "Recipients"), !isMassTransfer ? /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
  address: transferList[0].address,
  alias: transferList[0].name,
  withCopy: true,
  mt: "$5"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
  address: transferList[0].address
})) : /* @__PURE__ */ React__default["default"].createElement(reactUikit.Box, {
  sx: {
    "& > * + *": {
      borderTop: "1px dashed",
      borderColor: "main.$500"
    }
  },
  mt: "$5",
  p: 15,
  backgroundColor: "basic.$900",
  borderRadius: "$4",
  maxHeight: 240,
  overflowY: "auto"
}, transferList.map(({ address, amount, name }, i) => /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  key: i,
  alignItems: "center",
  justifyContent: "space-between",
  pt: i > 0 ? "7px" : 0,
  pb: i < transferList.length - 1 ? "7px" : 0
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressLabel, {
  address,
  alias: name,
  mt: "$5"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
  address
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, amount)))), attachment ? /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  mt: "$20",
  variant: "body2",
  color: "basic.$500"
}, "Attachment"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  mt: "$5",
  p: "15px",
  variant: "body2",
  color: "standard.$0",
  bg: "basic.$900",
  borderRadius: "$4"
}, attachment)) : null, isMassTransfer && /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "basic.$500",
  display: "block",
  mt: "$20",
  mb: "$5"
}, "Fee"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body2",
  color: "standard.$0"
}, transferFee)), meta && !isMassTransfer && /* @__PURE__ */ React__default["default"].createElement(FeeSelect, {
  mt: "$20",
  txMeta: meta,
  fee: tx.fee,
  onFeeSelect: handleFeeSelect,
  availableWavesBalance: userBalance
}))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(TransactionDetails, {
  tx
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
  data: txJSON
})))));
const SignTypedDataComponent = ({
  userAddress,
  userName,
  userBalance,
  data,
  onReject,
  onConfirm
}) => /* @__PURE__ */ React__default["default"].createElement(Confirmation, {
  address: userAddress,
  name: userName,
  balance: userBalance,
  onReject,
  onConfirm
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  px: "$40",
  py: "$20",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  alignItems: "center",
  justifyContent: "center",
  borderRadius: "circle",
  bg: "rgba(255, 175, 0, 0.1)",
  height: 60,
  width: 60
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Icon, {
  size: "60px",
  icon: reactUikit.iconSignMessage,
  color: "primary.$300"
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
  ml: "$20",
  flexDirection: "column"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1",
  color: "basic.$500"
}, "Off-chain"), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  fontSize: 26,
  lineHeight: "32px",
  color: "standard.$0"
}, "Sign Data"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tabs, null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabsList, {
  borderBottom: "1px solid",
  borderColor: "main.$700",
  mb: "$30",
  px: "$40",
  bg: "main.$900"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "Data")), /* @__PURE__ */ React__default["default"].createElement(reactUikit.Tab, {
  mr: "32px",
  pb: "12px"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.Text, {
  variant: "body1"
}, "JSON"))), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanels, {
  mb: "$30",
  px: "$40",
  bg: "main.$800"
}, /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataEntry, {
  data
})), /* @__PURE__ */ React__default["default"].createElement(reactUikit.TabPanel, null, /* @__PURE__ */ React__default["default"].createElement(DataJson, {
  data
})))));
const Preload = () => {
  return /* @__PURE__ */ React__default["default"].createElement(reactUikit.Flex, {
    backgroundColor: "main.$800",
    width: "520px",
    flexDirection: "column",
    justifyContent: "center",
    alignItems: "center",
    borderRadius: "$6",
    height: "584px"
  }, /* @__PURE__ */ React__default["default"].createElement(reactUikit.DotLoader, null));
};
function SignCustom(props) {
  return /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("div", {
    onClick: props.onCancel
  }), /* @__PURE__ */ React__default["default"].createElement("div", {
    className: "logo"
  }), /* @__PURE__ */ React__default["default"].createElement("div", null, props.title), /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("span", null, "Sign from"), /* @__PURE__ */ React__default["default"].createElement("span", null, /* @__PURE__ */ React__default["default"].createElement(reactUikit.AddressAvatar, {
    address: props.sender
  }))), props.data, /* @__PURE__ */ React__default["default"].createElement("div", null, /* @__PURE__ */ React__default["default"].createElement("button", {
    onClick: props.onCancel
  }, "Cancel"), /* @__PURE__ */ React__default["default"].createElement("button", {
    onClick: props.onConfirm
  }, "Ok")));
}
const transferUtils = __spreadValues({}, transferHelpers);
const hooks = __spreadValues({}, hooksModule);
const CONSTANTS = __spreadValues({}, constantsModule);
const utils = __spreadValues({}, utilsModule);
exports.CONSTANTS = CONSTANTS;
exports.Confirmation = Confirmation;
exports.DataEntry = DataEntry;
exports.DataJson = DataJson;
exports.FeeSelect = FeeSelect;
exports.GlobalCSS = GlobalCSS;
exports.Help = Help;
exports.IconTransfer = IconTransfer;
exports.InvokeFunction = InvokeFunction;
exports.InvokePayment = InvokePayment;
exports.Preload = Preload;
exports.SignAliasComponent = SignAliasComponent;
exports.SignBurn = SignBurn;
exports.SignCancelLeaseComponent = SignCancelLeaseComponent;
exports.SignCustom = SignCustom;
exports.SignDataComponent = SignDataComponent;
exports.SignInvoke = SignInvoke;
exports.SignIssueComponent = SignIssueComponent;
exports.SignLeaseComponent = SignLeaseComponent;
exports.SignMessageComponent = SignMessageComponent;
exports.SignReissueComponent = SignReissueComponent;
exports.SignSetAccountScriptComponent = SignSetAccountScriptComponent;
exports.SignSetAssetScript = SignSetAssetScript;
exports.SignSponsorshipComponent = SignSponsorshipComponent;
exports.SignTransfer = SignTransfer;
exports.SignTypedDataComponent = SignTypedDataComponent;
exports.TransactionDetails = TransactionDetails;
exports.batchPage = batchPage;
exports.hooks = hooks;
exports.iconTransferUtils = iconTransferUtils;
exports.transferUtils = transferUtils;
exports.utils = utils;
